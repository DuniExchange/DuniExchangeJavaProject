...
public class EncForm {
    ...
    public static String uploadFile(FileItem item, String relativeSavePath, HttpServletRequest request, ServletContext servletContext){
        System.out.println("destination path: " + relativeSavePath);
        String[] filenameSlipts = item.getName().split("\\.");
        ...
        String filename = ((UserAccount)request.getSession().getAttribute("currentAccount")).getUserUsername() + "-" + new SimpleDateFormat("ddMMyyyy-hhmmssSSS").format(new Date()) + "." + type; //change filename
